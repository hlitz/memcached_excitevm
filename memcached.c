...
//#include "/memcached/gcc-4.9.2/libitm/libitm.h"
#include "/memcached/tm-persistence/min-gcc/libitm/libitm.h"
...
static rel_time_t realtime(const time_t exptime) {
    ...
    if (exptime > REALTIME_MAXDELTA) {
        ...
        return (rel_time_t)(exptime + __transaction_atomic(tm_current_time));
    }
}
...
static int add_msghdr(conn *c)
{
    ...
    if (c->msgsize == c->msgused) {
        msg = /*excitevm_s*/realloc(c->msglist, c->msgsize * 2 * sizeof(struct msghdr));
        if (! msg)
            return -1;
        c->msglist = msg;
        c->msgsize *= 2;
...
static void conn_init(void) {
    freetotal = 200;
    freecurr = 0;
    if ((freeconns =  /*excitevm_s*/calloc(freetotal, sizeof(conn *))) == NULL) {
...
bool conn_add_to_freelist(conn *c) {
    ...
    if (freecurr < freetotal) {
        ...
        conn **new_freeconns = /*excitevm_s*/realloc(freeconns, sizeof(conn *) * newsize);
        if (new_freeconns) {
            freetotal = newsize;
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (NULL == c) {
        if (!(c = (conn *) /*excitevm_s*/calloc(1, sizeof(conn)))) { //assaf
            fprintf(stderr, " excitevm_scalloc()\n");
            return NULL;
        }
        ...
        c->hdrsize = 0;
//assaf
        c->rbuf = (char *)/*excitevm_s*/malloc((size_t)c->rsize);
        c->ritem_buf = (char *)/*excitevm_s*/malloc((size_t)c->wsize);
        c->wbuf = (char *)/*excitevm_s*/malloc((size_t)c->wsize);
        c->ilist = (item **)/*excitevm_s*/malloc(sizeof(item *) * c->isize);
        c->suffixlist = (char **)/*excitevm_s*/malloc(sizeof(char *) * c->suffixsize);
        c->iov = (struct iovec *)/*excitevm_s*/malloc(sizeof(struct iovec) * c->iovsize);
        c->msglist = (struct msghdr *)/*excitevm_s*/malloc(sizeof(struct msghdr) * c->msgsize);

        if (c->rbuf == 0 || c->wbuf == 0 || c->ilist == 0 || c->iov == 0 ||
                c->msglist == 0 || c->suffixlist == 0) {
            conn_free(c);
            fprintf(stderr, "excitevm_smalloc()\n");
            return NULL;
        }
...
static void conn_cleanup(conn *c) {
    ...
    if (c->write_and_free) {
        /*excitevm_s*/free(c->write_and_free);
        c->write_and_free = 0;
    }
...
void conn_free(conn *c) {
    if (c) {
        ...
        if (c->hdrbuf)
             /*excitevm_s*/free(c->hdrbuf);
        if (c->msglist)
             /*excitevm_s*/free(c->msglist);
        if (c->rbuf)
             /*excitevm_s*/free(c->rbuf);
        if (c->ritem_buf)
            free(c->ritem_buf);
        if (c->wbuf)
             /*excitevm_s*/free(c->wbuf);
        if (c->ilist)
             /*excitevm_s*/free(c->ilist);
        if (c->suffixlist)
             /*excitevm_s*/free(c->suffixlist);
        if (c->iov)
             /*excitevm_s*/free(c->iov);
         /*excitevm_s*/free(c);
    }
}
...
static void conn_shrink(conn *c) {
    ...
    if (c->rsize > READ_BUFFER_HIGHWAT && c->rbytes < DATA_BUFFER_SIZE) {
        ...
        newbuf = (char *)/*excitevm_s*/realloc((void *)c->rbuf, DATA_BUFFER_SIZE);
    ...
    if (c->isize > ITEM_LIST_HIGHWAT) {
        item **newbuf = (item**) /*excitevm_s*/realloc((void *)c->ilist, ITEM_LIST_INITIAL * sizeof(c->ilist[0]));
        if (newbuf) {
            c->ilist = newbuf;
            c->isize = ITEM_LIST_INITIAL;
        }
    ...
    if (c->msgsize > MSG_LIST_HIGHWAT) {
        struct msghdr *newbuf = (struct msghdr *)/*excitevm_s*/realloc((void *)c->msglist, MSG_LIST_INITIAL * sizeof(c->msglist[0]));
        if (newbuf) {
            c->msglist = newbuf;
            c->msgsize = MSG_LIST_INITIAL;
        }
    ...
    if (c->iovsize > IOV_LIST_HIGHWAT) {
        struct iovec *newbuf = (struct iovec *) /*excitevm_s*/realloc((void *)c->iov, IOV_LIST_INITIAL * sizeof(c->iov[0]));
        if (newbuf) {
            c->iov = newbuf;
            c->iovsize = IOV_LIST_INITIAL;
        }
...
static int ensure_iov_space(conn *c) {
    ...
    if (c->iovused >= c->iovsize) {
        int i, iovnum;
        struct iovec *new_iov = (struct iovec *)/*excitevm_s*/realloc(c->iov,
...
static int add_iov(conn *c, const void *buf, int len) {
    ...
    do {
        ...
        m->msg_iov[m->msg_iovlen].iov_base = malloc(sizeof(char)*len);
memcpy(m->msg_iov[m->msg_iovlen].iov_base,buf,len);
printf("BUF: %s\n",(char*)buf);
...
static int build_udp_headers(conn *c) {
    ...
    if (c->msgused > c->hdrsize) {
        void *new_hdrbuf;
        if (c->hdrbuf)
            new_hdrbuf = /*excitevm_s*/realloc(c->hdrbuf, c->msgused * 2 * UDP_HEADER_SIZE);
        else
            new_hdrbuf = /*excitevm_s*/malloc(c->msgused * 2 * UDP_HEADER_SIZE);
        if (! new_hdrbuf)
            return -1;
...
printf("ITEM: %s %d flags\n", ITEM_data(it), (it->it_flags & (ITEM_LINKED|ITEM_SLABBED)));
...
printf("ITEM0: %s", ITEM_data(it));
      ret = store_item(it, comm, c);
printf("ITEM1: %s", ITEM_data(it));
    ...
    case DELTA_ITEM_NOT_FOUND:
        if (req->message.body.expiration != 0xffffffff) {
            ...
            if (it != NULL) {
                ...
                char temp_buf[INCR_MAX_STORAGE_LEN];            
                snprintf(temp_buf/*ITEM_data(it)*/, INCR_MAX_STORAGE_LEN, "%llu",
                         (unsigned long long)req->message.body.initial);
__transaction_atomic { //assaf
                tm_memcpy(ITEM_data(it), temp_buf, INCR_MAX_STORAGE_LEN);
}
...
// [branch 011] wrap this to make it OK to call
__attribute__((transaction_pure))
static uint16_t tm_htons(uint16_t hostshort)
{
    return htons(hostshort);
}

// [branch 011] Need to make this transaction-safe
...
static void append_bin_stats(const char *key, const uint16_t klen,
    ...
    protocol_binary_response_header header = {
        ...
        // [branch 011] Need wrapped htons
        .response.keylen = (uint16_t)tm_htons(klen),
...
// [branch 011] We need a min function to get append_ascii_stats to work
//              correctly
#define min(x, y) (x) < (y) ? (x) : (y)

// [branch 011] Custom pure snprintf for writing "END"
__attribute__((transaction_pure))
...
// [branch 011] Forward declare this one
__attribute__((transaction_pure))
static int tm_snprintf_s(char *str, size_t size, const char *format,
                         const char *val);

// [branch 011] A second one with two strings
__attribute__((transaction_pure))
...
// [branch 011] Need this to be transaction_safe
__attribute__((transaction_safe))
static void append_ascii_stats(const char *key, const uint16_t klen,
    ...
    // [branch 011] the trick in this code is that we need to marshall
    //              parameters back and forth, in order to get the snprintf
    //              calls to work correctly
    //
    //              Note that we can't use alloca inside of this code, so we
    //              have to guess at a size.  4096 should be big enough
    char k[4096];
    char v[4096];
    // [branch 011] step 1: copy key and val onto stack.  Note that we can't
    //                      use klen and vlen, because they *could* be too
    //                      big if our guess of 4096 is off
    if (klen)
        tm_strncpy_to_local(k, key, min(room, 4096));
    if (vlen)
        tm_strncpy_to_local(v, val, min(room, 4096));

    // [branch 011] step 2: now we need a temporary destination, and we can
    //                      snprintf directly to it
    char p[8192];
    if (klen == 0 && vlen == 0) {
        // [branch 011] snprintf with no parameters and a non-literal format
        //              angers gcc, so we use a custom wrapping
        nbytes = tm_snprintf_end(p, min(room, 8192));
    } else if (vlen == 0) {
        // [branch 011] This reuses code from other safe stats work
        nbytes = tm_snprintf_s(p, min(room, 8192), "STAT %s\r\n", k);
    } else {
        // [branch 011] This required new code
        nbytes = tm_snprintf_s_s(p, min(room,8192), "STAT %s %s\r\n", k, v);
    }

    // [branch 011] step 3: Now move the result from p back into pos
    tm_strncpy(pos, p, min(room, 8192));

    c->stats.offset += nbytes;
}

// [branch 011] This needs to be transaction_safe
__attribute__((transaction_safe))
...
__transaction_atomic { //assaf
    ...
    while (needed > available) {
        // [branch 011] Need safe asserts here
        tm_assert(nsize > 0);
        nsize = nsize << 1;
    ...
    if (nsize != c->stats.size) {
        // [branch 011] This realloc needs to be safe, and we need to know
        //              the old size.  It appears to be c->stats_size...
        char *ptr = tm_realloc(c->stats.buffer, nsize, c->stats.size);
        if (ptr) {
            c->stats.buffer = ptr;
            c->stats.size = nsize;
        } else {
...
// [branch 011] Now that we have safe stats, we know that this needs to be
//              called from atomic transactions.  However, gcc didn't catch
//              this error earlier, so now we need to fix it
__attribute__((transaction_safe))
                  ...
                  const void *cookie)
{
    ...
    // [branch 011] Need safe asserts here
    tm_assert(c->stats.offset <= c->stats.size);
}
...
static void bin_read_key(conn *c, enum bin_substates next_substate, int extra) {
    ...
    if (c->rlbytes > c->rsize - offset) {
        ...
        if (nsize != c->rsize) {
            ...
            char *newm = /*excitevm_s*/realloc(c->rbuf, nsize);
            if (newm == NULL) {
                if (settings.verbose) {
                    fprintf(stderr, "%d: Failed to grow buffer.. closing connection\n",
                            c->sfd);
                }
printf("conn_closing3\n");
...
static void process_bin_flush(conn *c) {
    ...
    if (exptime > 0) {
        ...
        // [branch 006] Use an expression to access time
        settings.oldest_live = __transaction_atomic(tm_current_time) - 1;
    }
    item_flush_expired();
...
// [branch 012b] Oncommit handler for do_store_item's fprintf
static void dsi_fprintf1(void *param)
{
    unsigned long long *ulls = param;
    fprintf(stderr, "CAS:  failure: expected %llu, got %llu\n", ulls[0], ulls[1]);
     /*excitevm_s*/free(ulls);
}
...
tm_assert((it->it_flags & (ITEM_LINKED|ITEM_SLABBED)) == 0);
        ...
        || comm == NREAD_APPEND || comm == NREAD_PREPEND))
    {
        ...
        if(old_it == NULL) {
            ...
            if(settings.verbose > 1) {
                // [branch 012b] Move this to an oncommit call
                // NB: need to allocate room for the parameters
                unsigned long long* ulls = excitevm_smalloc(2*sizeof(unsigned long long));
                ulls[0] = (unsigned long long)ITEM_get_cas(old_it);
                ulls[1] = (unsigned long long)ITEM_get_cas(it);
                registerOnCommitHandler(dsi_fprintf1, ulls);
            }
            stored = EXISTS;
        }
    } else {
...
tm_assert((it->it_flags & (ITEM_LINKED|ITEM_SLABBED)));
...
tm_assert((it->it_flags & (ITEM_LINKED|ITEM_SLABBED)));
            ...
            __transaction_atomic { //assaf
                if (comm == NREAD_APPEND) {
                    ...
                    tm_memcpy(ITEM_data(new_it), ITEM_data(old_it), old_it->nbytes);
                    tm_memcpy(ITEM_data(new_it) + old_it->nbytes - 2 /* CRLF */, ITEM_data(it), it->nbytes);
                } else {
                    /* NREAD_PREPEND */
                    // [branch 009b] Use safe memcpy
                    tm_memcpy(ITEM_data(new_it), ITEM_data(it), it->nbytes);
                    tm_memcpy(ITEM_data(new_it) + it->nbytes - 2 /* CRLF */, ITEM_data(old_it), old_it->nbytes);
                }
...
void append_stat(const char *name, ADD_STAT add_stats, conn *c,
    ...
    // [branch 009] Switch to safe strlen
    add_stats(name, tm_strlen(name), val_str, vlen, c);
}

// [branch 011] Wrapped versions of snprintf to make it safe when doing all
...
// [branch 011b] NB: this is just for the b branch
...
// [branch 011] Clone append_stat to eliminate varargs, than make safe
void append_stat_llu(const char *name, ADD_STAT add_stats, conn *c,
    ...
    vlen = tm_snprintf_llu(val_str, sizeof(val_str) - 1, fmt, val);

    // [branch 009] Switch to safe strlen
    add_stats(name, tm_strlen(name), val_str, vlen, c);
}

// [branch 011] Clone append_stat to eliminate varargs, then make safe
void append_stat_u(const char *name, ADD_STAT add_stats, conn *c,
    ...
    vlen = tm_snprintf_u(val_str, sizeof(val_str) - 1, fmt, val);
...
void append_stat_d(const char *name, ADD_STAT add_stats, conn *c,
    ...
    vlen = tm_snprintf_d(val_str, sizeof(val_str) - 1, fmt, val);
...
void append_stat_lu(const char *name, ADD_STAT add_stats, conn *c,
    ...
    vlen = tm_snprintf_lu(val_str, sizeof(val_str) - 1, fmt, val);
...
void append_stat_ld(const char *name, ADD_STAT add_stats, conn *c,
    ...
    vlen = tm_snprintf_ld(val_str, sizeof(val_str) - 1, fmt, val);
...
void append_stat_s(const char *name, ADD_STAT add_stats, conn *c,
    ...
    // [branch 011] For char*, we need to marshall the string onto the stack,
    //              then pass it to snprintf
    char val_local[STAT_VAL_LEN];
    tm_strncpy_to_local(val_local, val, STAT_VAL_LEN);

    vlen = tm_snprintf_s(val_str, sizeof(val_str) - 1, fmt, val_local);
...
void append_stat_ld_ld(const char *name, ADD_STAT add_stats, conn *c,
    ...
    vlen = tm_snprintf_ld_ld(val_str, sizeof(val_str) - 1, fmt, val1, val2);
...
static void server_stats(ADD_STAT add_stats, conn *c) {
    pid_t pid = getpid();
    // [branch 006] Use an expression to access time
    rel_time_t now = __transaction_atomic(tm_current_time);
    ...
    // [branch 011] Hoist call out of transaction
    const char *cached_event_get_version = event_get_version();

    // [branch 002] Replaced STATS_LOCK with relaxed transaction
    // [branch 011] With safe stats, this can be atomic
    __transaction_atomic {

    // [branch 011] Use safe stats
    APPEND_STAT_LU("pid", "%lu", (long)pid);
    APPEND_STAT_U("uptime", "%u", now);
    APPEND_STAT_LD("time", "%ld", now + (long)process_started);
    APPEND_STAT_S("version", "%s", VERSION);
    // [branch 011] Note: hoisted call out of transaction
    APPEND_STAT_S("libevent", "%s", cached_event_get_version);
    APPEND_STAT_D("pointer_size", "%d", (int)(8 * sizeof(void *)));

#ifndef WIN32
    // [branch 011] These two are different, in that they have 2 arguments,
    //              but they're still not tricky
    append_stat_ld_ld("rusage_user", add_stats, c, "%ld.%06ld",
                (long)usage.ru_utime.tv_sec,
                (long)usage.ru_utime.tv_usec);
    append_stat_ld_ld("rusage_system", add_stats, c, "%ld.%06ld",
                (long)usage.ru_stime.tv_sec,
                (long)usage.ru_stime.tv_usec);
#endif /* !WIN32 */

    APPEND_STAT_U("curr_connections", "%u", stats.curr_conns - 1);
    APPEND_STAT_U("total_connections", "%u", stats.total_conns);
    if (settings.maxconns_fast) {
        // [branch 011] Use safe APPEND_STAT calls
        APPEND_STAT_LLU("rejected_connections", "%llu", (unsigned long long)stats.rejected_conns);
    }
    APPEND_STAT_U("connection_structures", "%u", stats.conn_structs);
    APPEND_STAT_U("reserved_fds", "%u", stats.reserved_fds);
    // [branch 011] Use safe APPEND_STAT calls
    APPEND_STAT_LLU("cmd_get", "%llu", (unsigned long long)thread_stats.get_cmds);
    APPEND_STAT_LLU("cmd_set", "%llu", (unsigned long long)slab_stats.set_cmds);
    APPEND_STAT_LLU("cmd_flush", "%llu", (unsigned long long)thread_stats.flush_cmds);
    APPEND_STAT_LLU("cmd_touch", "%llu", (unsigned long long)thread_stats.touch_cmds);
    APPEND_STAT_LLU("get_hits", "%llu", (unsigned long long)slab_stats.get_hits);
    APPEND_STAT_LLU("get_misses", "%llu", (unsigned long long)thread_stats.get_misses);
    APPEND_STAT_LLU("delete_misses", "%llu", (unsigned long long)thread_stats.delete_misses);
    APPEND_STAT_LLU("delete_hits", "%llu", (unsigned long long)slab_stats.delete_hits);
    APPEND_STAT_LLU("incr_misses", "%llu", (unsigned long long)thread_stats.incr_misses);
    APPEND_STAT_LLU("incr_hits", "%llu", (unsigned long long)slab_stats.incr_hits);
    APPEND_STAT_LLU("decr_misses", "%llu", (unsigned long long)thread_stats.decr_misses);
    APPEND_STAT_LLU("decr_hits", "%llu", (unsigned long long)slab_stats.decr_hits);
    APPEND_STAT_LLU("cas_misses", "%llu", (unsigned long long)thread_stats.cas_misses);
    APPEND_STAT_LLU("cas_hits", "%llu", (unsigned long long)slab_stats.cas_hits);
    APPEND_STAT_LLU("cas_badval", "%llu", (unsigned long long)slab_stats.cas_badval);
    APPEND_STAT_LLU("touch_hits", "%llu", (unsigned long long)slab_stats.touch_hits);
    APPEND_STAT_LLU("touch_misses", "%llu", (unsigned long long)thread_stats.touch_misses);
    APPEND_STAT_LLU("auth_cmds", "%llu", (unsigned long long)thread_stats.auth_cmds);
    APPEND_STAT_LLU("auth_errors", "%llu", (unsigned long long)thread_stats.auth_errors);
    APPEND_STAT_LLU("bytes_read", "%llu", (unsigned long long)thread_stats.bytes_read);
    APPEND_STAT_LLU("bytes_written", "%llu", (unsigned long long)thread_stats.bytes_written);
    APPEND_STAT_LLU("limit_maxbytes", "%llu", (unsigned long long)settings.maxbytes);
    APPEND_STAT_U("accepting_conns", "%u", stats.accepting_conns);
    APPEND_STAT_LLU("listen_disabled_num", "%llu", (unsigned long long)stats.listen_disabled_num);
    APPEND_STAT_D("threads", "%d", settings.num_threads);
    APPEND_STAT_LLU("conn_yields", "%llu", (unsigned long long)thread_stats.conn_yields);
    APPEND_STAT_U("hash_power_level", "%u", stats.hash_power_level);
    APPEND_STAT_LLU("hash_bytes", "%llu", (unsigned long long)stats.hash_bytes);
    APPEND_STAT_U("hash_is_expanding", "%u", stats.hash_is_expanding);
    if (settings.slab_reassign) {
        APPEND_STAT_U("slab_reassign_running", "%u", stats.slab_reassign_running);
        APPEND_STAT_LLU("slabs_moved", "%llu", stats.slabs_moved);
    }
    }
}
...
printf("ITEM_GET: %s\n", ITEM_data(it));
                if (i >= c->isize) {
                    item **new_list = /*excitevm_s*/realloc(c->ilist, sizeof(item *) * c->isize * 2);
                    if (new_list) {
                        c->isize *= 2;
                        c->ilist = new_list;
                    } else {
                ...
                if (return_cas)
                {
                  ...
                  if (i >= c->suffixsize) {
                    char **new_suffix_list = /*excitevm_s*/realloc(c->suffixlist,
...
__transaction_atomic { //assaf
    // [branch 011b] Use custom snprintf variant
    tm_snprintf_dad(buf, (unsigned long long)value);
    // [branch 009b] Use safe strlen
    res = tm_strlen(buf);
    // [branch 007] Use an expression to read refcount
    if (res + 2 > it->nbytes || __transaction_atomic(it->tm_refcount) != 1) { /* need to realloc */
        ...
        // [branch 009b] Use safe memcpy
//    __transaction_atomic { //assaf added
        tm_memcpy(ITEM_data(new_it), buf, res);
        tm_memcpy(ITEM_data(new_it) + res, "\r\n", 2);
...
//}
        ...
        // [branch 002] Replaced cache_lock critical section with relaxed transaction
        //              Note that in the original code, there was a FIXME on the
...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
        if (c->rbytes >= c->rsize) {
            ...
            ++num_allocs;
            char *new_rbuf = /*excitevm_s*/realloc(c->rbuf, c->rsize * 2);
            if (!new_rbuf) {
...
printf("HERE6: %d  %p %d %d %s\n",c->sfd,  c->rbuf, avail, res, strerror(errno));
        if (res > 0) {
//printf("tid6 = %ld\n", syscall(SYS_gettid));
//printf("GOTDATA1: %d\n", gotdata);
            // [branch 010] Replace per-thread stats lock with atomic transaction
...
//free(m->msg_iov);//assaf
printf("sendmsg: %zd %s %p\n", res, strerror(errno), (void*)m);
        if (res > 0) {
            // [branch 010] Replace per-thread stats lock with atomic transaction
            __transaction_atomic {
...
//                }
           ...
           char *new_ritem = malloc(c->wsize);
           res = read(c->sfd, new_ritem, c->rlbytes);
__transaction_atomic {
           tm_memcpy(c->ritem, new_ritem, c->rlbytes);
}
           free(new_ritem);
printf("conn_nread res: %d %d %p %p %p %p\n", res, errno, c->ritem, c->rbuf, c->wbuf, c->rcurr);
            if (res > 0) {
                // [branch 010] Replace per-thread stats lock with atomic transaction
                __transaction_atomic {
                c->thread->stats.bytes_read += res;
...
static void clock_handler(const int fd, const short which, void *arg) {
    ...
    if (monotonic) {
        ...
        if (clock_gettime(CLOCK_MONOTONIC, &ts) == -1)
            return;
        // [branch 006] Use a transaction to set the time
        __transaction_atomic { tm_current_time = (rel_time_t) (ts.tv_sec - monotonic_start); }
        return;
    }
#endif
    {
        ...
        // [branch 006] Use a transaction to set the time
        __transaction_atomic { tm_current_time = (rel_time_t) (tv.tv_sec - process_started); }
    }
}
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        case 'l':
            if (settings.inter != NULL) {
                ...
                char *p = /*excitevm_s*/malloc(len);
                if (p == NULL) {
                    fprintf(stderr, "Failed to allocate memory\n");
                    return 1;
                }
                snprintf(p, len, "%s,%s", settings.inter, optarg);
                 /*excitevm_s*/free(settings.inter);
                settings.inter = p;
            } else {
...
//    excitevm_enter();
    ...
    if (settings.inter)
       /*excitevm_s*/free(settings.inter);
    if (l_socket)
       /*excitevm_s*/free(l_socket);
    if (u_socket)
       /*excitevm_s*/free(u_socket);
