 ...
 */
// [branch 002] Get rid of slabs_lock
// [branch 010] Get rid of slabs_rebalance_lock
static int slab_rebalance_pseudolock;

// [branch 010] A simple trylock: if it's 1, return 0, else set it to 1 and
//              return 1
...
// [branch 010] A simple mechanism for locking, with an approximation of
...
// [branch 010] A simple mechanism for locking, with spinning
...
// [branch 010] Unlock the pseudo-lock
...
// [branch 009] This can be safe once realloc is safe
__attribute__((transaction_safe))
...
static int grow_slab_list (const unsigned int id) {
    ...
    if (p->slabs == p->list_size) {
        ...
        // [branch 009] Switch to safe realloc
        void *new_list = tm_realloc(p->slab_list, new_size * sizeof(void *), p->list_size);
...
/*@null@*/
// [branch 004] This function is called from a relaxed transaction
// [branch 009] This can be safe once we have safe realloc
__attribute__((transaction_safe))
...
bool get_stats(const char *stat_type, int nkey, ADD_STAT add_stats, void *c) {
    ...
    if (add_stats != NULL) {
        if (!stat_type) {
            ...
            // [branch 011] With safe stats, this becomes atomic
            __transaction_atomic {
            APPEND_STAT_LLU("bytes", "%llu", (unsigned long long)stats.curr_bytes);
            APPEND_STAT_U("curr_items", "%u", stats.curr_items);
            APPEND_STAT_U("total_items", "%u", stats.total_items);
            }
...
__transaction_atomic { //assaf
    ...
    for(i = POWER_SMALLEST; i <= *power_largest; i++) {
        ...
        if (p->slabs != 0) {
            ...
            // [branch 011] Replace calls with safe stats
            APPEND_NUM_STAT_U(i, "chunk_size", "%u", p->size);
            APPEND_NUM_STAT_U(i, "chunks_per_page", "%u", perslab);
            APPEND_NUM_STAT_U(i, "total_pages", "%u", slabs);
            APPEND_NUM_STAT_U(i, "total_chunks", "%u", slabs * perslab);
            APPEND_NUM_STAT_U(i, "used_chunks", "%u",
                            slabs*perslab - p->sl_curr);
            APPEND_NUM_STAT_U(i, "free_chunks", "%u", p->sl_curr);
            /* Stat is dead, but displaying zero instead of removing it. */
            APPEND_NUM_STAT_U(i, "free_chunks_end", "%u", 0);
            APPEND_NUM_STAT_LLU(i, "mem_requested", "%llu",
                            (unsigned long long)p->requested);
            APPEND_NUM_STAT_LLU(i, "get_hits", "%llu",
                    (unsigned long long)thread_stats.slab_stats[i].get_hits);
            APPEND_NUM_STAT_LLU(i, "cmd_set", "%llu",
                    (unsigned long long)thread_stats.slab_stats[i].set_cmds);
            APPEND_NUM_STAT_LLU(i, "delete_hits", "%llu",
                    (unsigned long long)thread_stats.slab_stats[i].delete_hits);
            APPEND_NUM_STAT_LLU(i, "incr_hits", "%llu",
                    (unsigned long long)thread_stats.slab_stats[i].incr_hits);
            APPEND_NUM_STAT_LLU(i, "decr_hits", "%llu",
                    (unsigned long long)thread_stats.slab_stats[i].decr_hits);
            APPEND_NUM_STAT_LLU(i, "cas_hits", "%llu",
                    (unsigned long long)thread_stats.slab_stats[i].cas_hits);
            APPEND_NUM_STAT_LLU(i, "cas_badval", "%llu",
                    (unsigned long long)thread_stats.slab_stats[i].cas_badval);
            APPEND_NUM_STAT_LLU(i, "touch_hits", "%llu",
...
void *slabs_alloc(size_t size, unsigned int id) {
    ...
    // [branch 002] Replace slabs lock critical section with relaxed
    //              transaction
    // [branch 009] Safe realloc lets this become safe
    __transaction_atomic {
...
// [branch 012] Hoist fprintf out of transaction for oncommit call
static void srs_fprintf1(void *param)
{
...
static int slab_rebalance_start(void) {
    ...
    //              transaction
    // [branch 012] With oncommit of fprintf, this becomes atomic
    __transaction_atomic {
    // [branch 002] We can simply elide the slabs_lock, since we're already in
    ...
    // [branch 006] Renamed this var
    tm_slab_rebalance_signal = 2;

    if (settings.verbose > 1) {
        // [branch 012] move fprintf to oncommit
        registerOnCommitHandler(srs_fprintf1, NULL);
    }
...
// [branch 012] Move the fprintf to an oncommit handler... be sure to free
// the parameter in this case!
static void srm_fprintf1(void *param)
{
    int* ip = (int*)param;
...
static int slab_rebalance_move(void) {
    ...
    for (x = 0; x < slab_bulk_check; x++) {
        ...
        if (it->slabs_clsid != 255) {
            // [branch 003b] we won't acquire a lock, so hold_lock is not
            //               needed.  This results in us never falling into
            //               MOVE_LOCKED state, but that's OK.
            {
                ...
                if (refcount == 1) { /* item is unlinked, unused */
                    ...
                    if (settings.verbose > 2) {
                        ...
                        registerOnCommitHandler(srm_fprintf1, newdata);
                    }
                    status = MOVE_BUSY;
                }
                // [branch 003b] No lock to release anymore...
            }
        }
        ...
        switch (status) {
            ...
            case MOVE_BUSY:
                // [branch 007] use new tm refcounts
                tm_refcount_decr(&it->tm_refcount);
            case MOVE_LOCKED:
...
static void slab_rebalance_finish(void) {
    ...
    // [branch 002] Replaced cache_lock lock critical section with relaxed
    //              transaction
    // [branch 008] With safe asserts, this becomes atomic
    __transaction_atomic {
    // [branch 002] We can simply elide the slabs_lock, since we're already in
    ...
    // [branch 006] Renamed this variable
...
static int slab_automove_decision(int *src, int *dst) {
    ...
    if (__transaction_atomic(tm_current_time) >= next_run) {
        next_run = __transaction_atomic(tm_current_time) + 10;
    } else {
        return 0;
    }
...
excitevm_enter();
    ...
    while (__transaction_atomic(tm_do_run_slab_thread)) {
...
void stop_slab_maintenance_thread(void) {
    ...
    __transaction_atomic {
    // [branch 006] These two variables have been renamed since they are no longer volatile
    tm_do_run_slab_thread = 0;
    tm_do_run_slab_rebalance_thread = 0;
    // [branch 010] Removed signal on maintenance_cond
    }
