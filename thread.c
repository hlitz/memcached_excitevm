/* -*- mode: C; tab-width: 4; c-basic-offse[: 4; indent-tabs-mode: nil -*- */
/*
...
// [branch 003b] Removed item_global_lock
...
printf("thread_libevent_proces\n");
    ...
    me->new_conn_queue = /*excitevm_s*/malloc(sizeof(struct conn_queue));
    ...
    // [branch 010] No per-thread stats mutexes to initialize anymore...
...
item *item_get(const char *key, const size_t nkey) {
    ...
    uint32_t hv;
    hv = hash(key, nkey, 0);
    // [branch 003b] Replaced item_lock with transaction
    // [branch 012b] With fprintf oncommit, this becomes atomic
    __transaction_atomic {
...
int item_link(item *item) {
    ...
    // [branch 003b] Replaced item_lock with transaction
    // [branch 012b] With fprintf oncommit, this becomes atomic
assert((item->it_flags & (ITEM_LINKED|ITEM_SLABBED)) == 0);
assert(0);
    __transaction_atomic {
...
void item_remove(item *item) {
    // [branch 003b] Replaced item_lock with transaction
    // [branch 008b] With safe asserts, this becomes atomic
    __transaction_atomic {
    do_item_remove(item);
    }
}
...
void item_unlink(item *item) {
    ...
    hv = hash(ITEM_key(item), item->nkey, 0);
    // [branch 003b] Replaced item_lock with transaction
    // [branch 009b] This is now atomic
    __transaction_atomic {
...
// [branch 002] Removed functions for acquiring and releasing stats_lock
void threadlocal_stats_reset(void) {
    int ii, sid;
    for (ii = 0; ii < settings.num_threads; ++ii) {
        // [branch 010] Replace per-thread stats lock with transaction
        __transaction_atomic {
...
void thread_init(int nthreads, struct event_base *main_base) {
    ...
    // [branch 003b] No need to initialize the item locks anymore
    pthread_key_create(&item_lock_type_key, NULL);
    // [branch 003b] No need to initialize item_global_lock anymore

    threads =  excitevm_scalloc(nthreads, sizeof(LIBEVENT_THREAD));
    if (! threads) {
